...
신청case
고객이 창고를 신청요청한다.(ok)
창고관리 시스템이 창고 할당을 요청한다.(ok)
창고 자동 할당이 완료된다.(ok)
신청상태 및 할당상태를 갱신한다.(ok)
고객에게 알림을 한다.(ok)


취소case
고객이 신청한 창고대여를 취소요청한다.(ok)
창고관리 시스템이 창고 할당 취소를 요청한다.(ok)
창고 할당이 취소된다.(ok)
취소상태로 갱신한다.(ok)
고객에게 알림을 한다.(ok)

확인case
고객이 신청 진행내역을 볼 수 있어야 한다. (ok) 
...
![image](https://user-images.githubusercontent.com/78134087/109809443-a403f600-7c6b-11eb-87c9-8563c5a39612.png)
